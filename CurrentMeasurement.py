    ...
    def abc(self):
        ...
        for i in range (0,20):
            ...
            if read_data != b'':
                ...
                s.send(b' * Max:')
                s.send(str(number1).encode('utf-8'))
                s.send(b' * Min:')
                s.send(str(number2).encode('utf-8'))
                s.send(b' uA \n')
            time.sleep(0.02)
                            
    def portConnect(self):
        recivedTcp = s.recvfrom(4096)
        if recivedTcp != b'':
            self.counter = 0
            self.abc()
                
                #data.append('' + str(i+1) + '.SampleCurrent: ' + str(number) + '')
                #self.textEditReceiveData.setText("\n".join(data))                
...
ppk2_test = PPK2_API("COM8")  # serial port will be different for you
